# JAEHAN JANG

Backend & Full-Stack Developer  
서비스의 구조와 흐름을 이해하고, 사용자 입장에서 가장 편리한 개발을 지향합니다.

---

## 🛠 Tech Stack
- **Backend**: Java, Spring Boot, Node.js, Express
- **Frontend**: React, Vite, JavaScript, HTML/CSS
- **Database**: MySQL
- **DevOps / Infra**: Nginx, Docker, PM2
- **AI / Data**: Python, TensorFlow, U-Net, BERT
- **Tools**: Git, GitHub, VS Code, Figma

---

## 📂 Projects

### 🛒 GCShop (가천샵)  
> 대학 수업 프로젝트로 제작한 쇼핑몰 웹 서비스 

- 상품 조회, 회원 관리, 장바구니, 구매 기능 구현
- 웹과 데이터베이스 연동 구조 이해
- MVC 기반 서버 로직 설계 경험

🔗 Repository & Demo : https://github.com/JAEHAN-JANG/gcshop

---

### 🌱 Carbon Neutral Platform (P-Project)  
> 탄소 배출량 계산 및 중립화 지원 웹 서비스

- 사용자 입력 기반 탄소 배출량 계산
- 평균 데이터 비교 및 시각화
- 팀 프로젝트에서 **백엔드 및 데이터 처리 담당**

🔗 Repository: *(준비 중)*  
🔗 Demo: *(준비 중)*

---

### 🐶🐱 멍냥케어 (졸업 프로젝트)  
> 반려동물 피부 질환 예측 및 케어 지원 AI 서비스

- 반려동물 피부 이미지 기반 질환 분류
- 중증도 판단 및 병원/약품 추천
- AI 모델 학습부터 서비스 연동까지 전체 경험

🔗 Repository: *(준비 중)*  
🔗 Demo: *(준비 중)*

---

### 🏢 WellBridge Homepage  
> 실무 환경에서 진행한 기업 홈페이지 및 관리자 시스템 개발

- React + Spring Boot 기반 서비스
- 관리자 인증, 파일 업로드, 콘텐츠 관리 기능 구현
- **회사 코드 비공개 / 케이스 스터디 형태로 정리**

🔗 Case Study: *(준비 중)*

---

## 📌 Contact
- GitHub: https://github.com/JAEHAN-JANG
- Email: (원하면 나중에 추가)
